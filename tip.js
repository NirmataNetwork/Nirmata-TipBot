	...
	if (isCallingBot(msg.content) == true) {
		...
		if (isBotListening == false && (msg.author.id == owner_id_1 || msg.author.id == owner_id_2)) {
			if (command == "startlistening") {
				isBotListening = true;
				msg.channel.send("Bot returned to life again");
			}
		}
		...
		switch (command) {
			...
			case 'network':
				...
				msg.channel.send(
					"Whoops! Please try again later. Current network height: " + (Daemon.height) + " 😄"
				);
				break;
			case 'help':
				msg.channel.send(
						...
						.then((collected) => {
							if (collected.first().content == "yes") {
								withDraw(msg.author.id, arguments[2], arguments[3], function (success, txhash) {
									if (success == true) {
										msg.author.send("Your withdrawal request was successfuly executed and your funds are on the way :money_with_wings: . TxHash is https://explorer.nirmata-network.com/transaction/" + txhash);
									} else {
			...
			case 'switchtipreceive':
				...
				case 'distribute':
					...
					if (isNaN(totalCoins) || totalCoins <= 0) {
						msg.reply("Specify the correct number of coins to distribute.");
						return;
					}
					...
					msg.guild.fetchMembers().then(guild => {
						...
						const nirUserRole = guild.roles.find(role => role.name === "NiR User");
						if (!nirUserRole) {
							msg.reply("Role 'NiR User' not found on the server.");
							return;
						}
				
						const onlineMembersWithRole = guild.members.filter(member => 
							member.roles.has(nirUserRole.id) && // Проверяем, есть ли у участника нужная роль
							member.presence.status === 'online' && // Проверяем, что участник онлайн
							!member.user.bot // Исключаем ботов
						);
				
						if (onlineMembersWithRole.size === 0) {
							msg.reply("There are no online users with the 'NiR User' role on the server.");
							return;
						}
						...
						onlineMembersWithRole.forEach(member => {
							...
							getBalance(msg.author.id, msg, function (data) {
								if (!data) {
									msg.reply("Failed to retrieve your balance.");
									return;
								}
				
								TipSomebody(msg, msg.author.id, tiptarget, member.user.username, myname, coinsPerUser, function (success, message) {
									if (success) {
										msg.channel.send(`<@${tiptarget}> has been tipped ${formatDisplayBalance(coinsPerUser)} ${coin_name} :moneybag: by ${msg.author}`);
										msg.author.send(`Current balance is ${formatDisplayBalance(data.balance - coinsPerUser)} ${coin_name}!\n<@${tiptarget}> has been successfully tipped ${formatDisplayBalance(coinsPerUser)} ${coin_name}!`);
										// Отправка личного сообщения пользователю о получении чаевых
										member.user.send(`You have received ${formatDisplayBalance(coinsPerUser)} ${coin_name} in tips from ${msg.author.username}!`);
									} else {
						...
						msg.channel.send(`Each online user with the 'NiR User' role has received ${coinsPerUser.toFixed(2)} ${coin_name} coins.`);
					}).catch(error => {
						console.error("Failed to fetch members:", error);
						msg.reply("Failed to execute the command due to an internal error.");
					});
					break;
		}
	}
